...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Use fine-grained bucketing. No effect if --no-run is enabled.",
)
parser.add_argument(
    "-o",
    "--output",
...
p = subprocess.run(
    [executable, "--gtest_list_tests"], capture_output=True, encoding="utf-8"
)
p.check_returncode()
...
def advanced_bucketize(p: subprocess.CompletedProcess):
    if p.returncode == 0:
        ...
        top_level_bucket = (
            signal_to_name(-p.returncode) if p.returncode < 0 else "ERROR"
        )
        if top_level_bucket not in report.keys():
            report[top_level_bucket] = {}
        if "Check failed: Data_Namespace::CPU_LEVEL" in p.stderr:
            add_to_bucket(report[top_level_bucket], "MEMORY_LEVEL", test)
        elif (
            "Abort /opt/src/l0_gpu_driver/shared/source/memory_manager/memory_manager.cpp"
            in p.stdout
        ):
