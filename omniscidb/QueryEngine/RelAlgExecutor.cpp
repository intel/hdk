...
void RelAlgExecutor::executeStepWithPartitionedAggregation(const hdk::ir::Node* step_root,
  ...
  // Currently, partitioning is supported for materialized data only.
  // Materialize data here if required.
  ...
  part_agg->setBufferEntryHint(max_partition_size * 2);
  VLOG(1) << "Using buffer entry count hint for partitioned aggregation: "
          << part_agg->bufferEntryHint();
