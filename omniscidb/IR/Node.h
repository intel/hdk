...
class LeftDeepInnerJoin : public Node {
 public:
  LeftDeepInnerJoin(const std::shared_ptr<Filter>& filter,
                    NodeInputs inputs,
                    std::vector<std::shared_ptr<const Join>>& original_joins);

  const Expr* getInnerCondition() const;
  ExprPtr getInnerConditionShared() const;

  const Expr* getOuterCondition(const size_t nesting_level) const;
  ExprPtr getOuterConditionShared(const size_t nesting_level) const;
  ...
  bool coversOriginalNode(const Node* node) const;

  const Filter* getOriginalFilter() const;

  std::vector<std::shared_ptr<const Join>> getOriginalJoins() const;

 private:
  ExprPtr condition_;
  std::vector<ExprPtr> outer_conditions_per_level_;
  const std::shared_ptr<Filter> original_filter_;
  const std::vector<std::shared_ptr<const Join>> original_joins_;
};

// The 'Compound' node combines filter and on the fly aggregate computation.
// It's the result of combining a sequence of 'Filter' (optional), 'Project',
// 'Aggregate' (optional) and a simple 'Project' (optional) into a single node
// which can be efficiently executed with no intermediate buffers.
class Compound : public Node {
 public:
  Compound(ExprPtr filter,
           ExprPtrVector exprs,
           const size_t groupby_count,
           ExprPtrVector groupby_exprs,
           const std::vector<std::string>& fields,
           const bool is_agg)
...
class TableFunction : public Node {
 public:
  ...
  const Expr* getTableFuncInputExprAt(size_t idx) const {
    CHECK_LT(idx, table_func_input_exprs_.size());
    return table_func_input_exprs_[idx].get();
  }

  const ExprPtrVector& getTableFuncInputExprs() const { return table_func_input_exprs_; }

  void setTableFuncInputs(ExprPtrVector new_inputs) {
  ...
  std::shared_ptr<Node> deepCopy() const override {
    return std::make_shared<TableFunction>(*this);
  }
  ...
  size_t toHash() const override {
    if (!hash_) {
      hash_ = typeid(TableFunction).hash_code();
 ...
 private:
  ...
  ExprPtrVector col_input_exprs_;
  ExprPtrVector table_func_input_exprs_;
