...
static void dictionary_materialize(benchmark::State& state) {
  ...
  auto drop_create_table = [&]() {
    storage.dropTable("test_table", /*throw_if_not_exist=*/false);
    return storage.createTable(
        "test_table",
        {{"x", ctx.int64()}, {"dict", ctx.extDict(ctx.text(), 0, state.range(0))}});
  };

  for (auto _ : state) {
    state.PauseTiming();  // Stop timers. They will not count until they are resumed.
    const auto table_info = drop_create_table();
    storage.appendArrowTable(csv_table, "test_table");
    CHECK(table_info);
...
BENCHMARK(import_csv)->Unit(benchmark::kMillisecond)->Iterations(30);
BENCHMARK(dictionary_materialize)
