...
std::shared_ptr<StringDictionary> create_str_dict(const int32_t dict_id,
  ...
  std::shared_ptr<StringDictionary> string_dict =
      std::make_shared<StringDictionary>(dict_ref, materialize_hashes);
  return string_dict;
}
...
(benchmark::State& state) {
  for (auto _ : state) {
    ...
    StringDictionary string_dict(dict_ref, true);
  }
}
...
(benchmark::State& state) {
  const auto string_dict =
      create_and_populate_str_dict(1, false, append_strings_10M_10M_10);
...
(benchmark::State& state) {
  const auto source_string_dict =
      create_and_populate_str_dict(1, true, append_strings_10M_10M_10);
  const auto dest_string_dict =
      create_and_populate_str_dict(2, true, append_strings_10M_10M_10_randomized);
