...
cdef class Calcite:
  ...
  def __cinit__(self, SchemaProvider schema_provider, Config config, **kwargs):
    ...
    self.calcite = CalciteMgr.get(schema_provider.c_schema_provider, config.c_config, udf_filename, calcite_max_mem_mb)
  ...
  def process(self, string sql, **kwargs):
    ...
    return self.calcite.process(db_name, sql, filter_push_down_info, legacy_syntax, is_explain, is_view_optimize)
